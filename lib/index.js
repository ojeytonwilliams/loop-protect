...
var protect = function protect(t, timeout, extra, iterations) {
  return function (path) {
    ...
    if (t.isVariableDeclaration(body)) {
      ...
      // path.container.filter(x => x.type === 'VariableDeclaration').map(x => x.declarations[0].init.callee ? x.declarations[0].init.callee.body.body.unshift(inside) : x.declarations[0].init.body.body.unshift(inside));
    }
